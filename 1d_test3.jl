@time begin
    ...
    function mlp_v(t, T, z, gamma, sigma, level, M)
        ...
        for l in 1:(level - 1)
            ...
            for i in 1:ns
                ...
                temp += (1-mmt)*my_C(beta, t, T)*picard_iter(v1[2], v2[2], min(l^(0.5), drift_b), min((l-1)^(0.5), drift_b), offset) * [sqrt(my_S - t), my_B / sigma] + mmt * (test1-test2)
                # if l==1
    ...
    function mlp_dist2(t, T, z, gamma, sigma, level, M)
        output = zeros(dim + 1)
        if level == 0
            return output
        end
    ...
    for i in 1:10
        result = @distributed (+) for j=1:M
            mlp_dist2(0, ub, z, gamma, sigma, level, M)
        end
        println(result/M, "\n")
    end
    ...
    println("T = ", ub, "; drift upper bound = ", drift_b, "; momentum = ", mmt, "; discount factor = ", beta, "; initial state = ", z, "; drift = ", gamma, "; sigma = ", sigma, "; level = ", level, "; M1 = ", M1, "; M2 = ", M2)
    # # #println(V, ", ", DV1(z))
    
    # test_array1 = []
    # test_array2 = []
    # for i in 1:10
    #     temp = my_mlp_v(0, ub, z, gamma, sigma, level, M1, M2)
    #     println(temp)
