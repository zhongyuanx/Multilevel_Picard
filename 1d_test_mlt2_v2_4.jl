@time begin
    ...
    function mlp_v(t, T, z, gamma, sigma, level, M)
        ...
        for l in 1:(level - 1)
            ...
            for i in 1:ns
                ...
                temp += my_C(beta, t, T)*picard_iter(v1[2], v2[2], min((l)^(0.5), drift_b), min((l-1)^(0.5), drift_b)) * [sqrt(my_S - t), my_mmt*my_B / sigma] + [0, (rate-my_mmt) * (test1[2]-test2[2])]
    ...
    println("T = ", ub, "; drift upper bound = ", drift_b, "; momentum = ", mmt, "; discount factor = ", beta, "; initial state = ", z, "; drift = ", gamma, "; sigma = ", sigma, "; v2")  #, "; M2 = ", M2)
    # # #println(V, ", ", DV1(z))

    for level in 8:8
        test_array1 = []
        test_array2 = []
        println("level = ", level, "; M = ", 8)
        for i in 1:10
            #temp = mlp_mlt_test(0, ub, z, gamma, sigma, level, M)
            temp = mlp_mlt(0, ub, z, gamma, sigma, level, 8)
