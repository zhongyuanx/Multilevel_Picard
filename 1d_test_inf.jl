@time begin
    ...
    function my_g(v, drift_bd)
        return -drift_bd * (max(v[2] - c, 0)) #- v[1]*0.01
    end
    ...
    function mlp_v(t, T, z, gamma, sigma, level, M)
        ...
        for i in 1:ns
            ...
            my_S = my_rtime(ub, 0.0, T-t)
            #my_S = my_rtime3(0.0, T-t, myshape)
            my_tau = bm_hitting(z, gamma, sigma, 0.0)
            my_B, my_R = stopped_rbm(z, gamma, my_tau, my_S/beta*ub, sigma, 0.0)
            #output += (1-mmt)*(cost(my_R))*[sqrt(my_S - t), my_B / sigma]
            output += cost(my_R) * [1.0*(my_S)^(1.0-myshape)/beta*ub, mmt*my_B / sigma / (my_S)^myshape]
        end
        #output = output / (ns) * my_C2(1.0, 0, T-t) #+ mmt*[z, 1]
        output = output / (ns) * my_C(ub, 0, T-t)
        ...
        for l in 1:(level - 1)
            ...
            for i in 1:ns
                ...
                temp += my_C(ub, 0, T-t)*picard_iter(v1, v2, min(l, drift_b), min(l-1, drift_b)) * [1.0*(my_S)^(1.0-myshape)/beta*ub, mmt*my_B / sigma / (my_S)^(myshape)] + [0, (1.0-mmt)*(test1[2]-test2[2])]
                #temp += my_C3(0.0, T-t, myshape)*picard_iter(v1, v2, drift_b, drift_b) * [1.0*(my_S)^(1.0-myshape)/beta, mmt*my_B / sigma / (my_S)^(myshape)] + [0, (1.0-mmt)*(test1[2]-test2[2])]
            end
    ...
    function mlp_mlt_call(t, T, z, gamma, sigma, level, M, thread_id, NUM_THREADS)
        ...
        for _ in 1:loop_num
            ...
            my_tau = bm_hitting(z, gamma, sigma, 0.0)
            my_B, my_R = stopped_rbm(z, gamma, my_tau, my_S/beta*ub, sigma, 0.0)
            output2 += (cost(my_R))*[1.0*(my_S)^(1.0-myshape)/beta*ub, mmt*my_B / sigma / (my_S)^(myshape)]
            #output += cost(my_R) * [sqrt(my_S - t), my_B / sigma]
        end
        #output2 = output2 / (ns) * my_C2(1.0, 0.0, T-t) #+ mmt*[z, 1]
        output2 = output2 / (ns) * my_C(ub, 0.0, T-t)
        ...
        for l in 1:(level - 1)
            ...
            for _ in 1:loop_num
                ...
                test1 = mlp_v(t, T, z, gamma, sigma, l, M)
                test2 = mlp_v(t, T, z, gamma, sigma, l-1, M)
                temp += my_C(ub, 0.0, T-t)*picard_iter(v1, v2, min(l, drift_b), min(l-1, drift_b)) * [1.0*(my_S)^(1.0-myshape)/beta*ub, mmt*my_B / sigma / (my_S)^(myshape)] + [0, (1.0-mmt) * (test1[2]-test2[2])]
                #temp += my_C3(0.0, T-t, myshape)*picard_iter(v1, v2, drift_b, drift_b) * [1.0*(my_S)^(1.0-myshape)/beta, mmt*my_B / sigma / (my_S)^(myshape)] + [0, (1.0-mmt) * (test1[2]-test2[2])]
            end
    ...
    for x in 0.6
        for level in 1:4
            ...
            for i in 1:5
                ...
                temp = mlp_mlt(0, 1.0, x, gamma, sigma, level, M[level])
