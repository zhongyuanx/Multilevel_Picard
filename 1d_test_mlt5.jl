@time begin
    ...
    function mlp_v(t, T, z, gamma, sigma, level, M)
        ...
        for l in 1:(level - 1)
            ...
            for i in 1:ns
                ...
                temp += (1-mmt)*my_C(beta, t, T)*picard_iter(v1[2], v2[2], min(2.0*l^(0.5), drift_b), min(2.0*(l-1)^(0.5), drift_b)) * [sqrt(my_S - t), my_B / sigma] + mmt * (test1-test2)
            end
    ...
    function mlp_mlt_incr_call(t, T, z, gamma, sigma, level, M, thread_id, NUM_THREADS)
        ...
        for _ in 1:loop_num
            ...
            output3 += (1-mmt)*my_C(beta, t, T)*picard_iter(v1[2], v2[2], min(level-1, drift_b), min(level-2, drift_b)) * [sqrt(my_S - t), my_B / sigma] #+ mmt * (test1-test2)
        end
    ...
    function mlp_mlt_call(t, T, z, gamma, sigma, level, M, thread_id, NUM_THREADS)
        ...
        for _ in 1:loop_num
            ...
            my_tau = bm_hitting(z, gamma, sigma, t)
            my_B, my_R = stopped_rbm(z, gamma, my_tau, my_S, sigma, t)
            output2 += (1-mmt)*(cost(my_R))*[sqrt(my_S - t), my_B / sigma]
            #output += cost(my_R) * [sqrt(my_S - t), my_B / sigma]
        end
