@time begin
    ...
    function my_g(v::Array{Float64}, drift_bd::Float64)
        return -penalty1*max(v[1],0)+drift_bd * min(c - v[2], 0)
    end
    
    function picard_iter(v1::Array{Float64}, v2::Array{Float64}, drift_bd1::Float64, drift_bd2::Float64)
    ...
    function mlp_v(t, T, z, gamma, sigma, level, M)
        ...
        for l in 1:(level - 1)
            ...
            for i in 1:ns
                ...
                #temp += my_C(beta, t, T)*picard_iter(v1, v2, drift_b, drift_b) * [sqrt(my_S - t), my_B / sigma]
                #try an Anderson scheme
                if l==1
                    temp += mmt*my_C(beta, t, T)*picard_iter(v1, v2, min(l^(0.4), drift_b), min((l-1)^(0.4), drift_b)) * [sqrt(my_S - t), my_B / sigma]
                    #temp += rate*my_C(beta, t, T)*picard_iter(v1[2], v2[2], min(l^(0.4), drift_b), min((l-1)^(0.4), drift_b)) * [sqrt(my_S - t), my_B / sigma]
                else
                    ...
                    temp += mmt*my_C(beta, t, T)*picard_iter(v1, v2, min(l^(0.4), drift_b), min((l-1)^(0.4), drift_b)) * [sqrt(my_S - t), my_B / sigma] + (1.0-mmt) * (test1-test2)
                end
            end
    ...
    println("T = ", ub, "; drift upper bound = ", drift_b, "; rate = ", rate, "; discount factor = ", beta, "; initial state = ", z, "; drift = ", gamma, "; sigma = ", sigma) #"; level = ", level, "; M = ", M) #, "; M2 = ", M2)
    # # #println(V, ", ", DV1(z))
